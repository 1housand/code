...
public class Main {
	...
	static Maze[][] maze; //The array board for the maze
	static int sizeR, sizeC; //Amount of rows and columns
	static Stack<Maze> backTrack = new Stack<Maze> (); //Backtracking stack
	static Random rand = new Random(); //Random number
	static int rand_Num;
	static Maze marker; //current place of the maze runner
	static String outputFile;
	static Stack<Maze>	path = new Stack<Maze> ();
	
	//If no dimensions are take in then uses default settings
	public static void main(String[] args){
		if (args.length==0) {System.out.println("A 20x20 random maze will be printed to output.txt");
			sizeR=20;
			sizeC=20;
			maze=new Maze[sizeR][sizeC];
			...
			maze=new Maze[sizeR][sizeC]; //create the maze board with the user's dimensions
	...
	private static void mazeRunner(){
		...
		for(int i=0; i<sizeR; i++){ //Initialize the maze array
			for(int j=0; j<sizeC; j++){
				maze[i][j]=new Maze(); //create Maze object in each array index
				maze[i][j].r=i;
				maze[i][j].c=j;
	...
	static void path(){
		Stack<Maze> backTrack2 = new Stack<Maze> ();
